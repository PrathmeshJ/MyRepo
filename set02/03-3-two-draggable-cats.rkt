...
(define (cat-after-mouse-and-key-event c mx my mev kev)
  (cond
    [(and (mouse=? mev "button-down") (key=? kev "left")) (cat-after-button-down c mx my)]
    [(and (mouse=? mev "button-down") (key=? kev "right")) (cat-after-drag c mx my)]
    [else c]))
...
(define (cat-after-button-up c x y)
  (if (cat-selected? c)
      (make-cat (- (cat-x-pos c) CATSPEED) (cat-y-pos c) false)
      c))
