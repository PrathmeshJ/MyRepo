...
(define (cat-after-tick-helper x-pos y-pos selected? dir)
  (cond
   [selected? (make-cat x-pos y-pos selected? dir)]
...
(define(check-south-boundary x-pos y-pos selected? dir)
  (if (< y-pos (- CANVAS-HEIGHT (/ (image-height CAT-IMAGE) 2) CATSPEED)) 
...
(define(check-north-boundary x-pos y-pos selected? dir)
  (if (> y-pos (/ (image-height CAT-IMAGE) 2)) 
      (make-cat x-pos (- y-pos CATSPEED) selected? dir)
      (make-cat x-pos y-pos selected? "SOUTH")))

(define(check-east-boundary x-pos y-pos selected? dir)
  (if (< x-pos (- CANVAS-WIDTH (/ (image-width CAT-IMAGE) 2) CATSPEED))
...
(define(check-west-boundary x-pos y-pos selected? dir)
  (if (> x-pos (/ (image-width CAT-IMAGE) 2)) 
      (make-cat (- x-pos CATSPEED) y-pos selected? dir)
...
;; EXAMPLES : (on-key-event-helper-for-left (make-cat 100 100 true "SOUTH") (make-cat 300 10 false "WEST"))
;;            (make-world (make-cat 100 100 true "WEST") (make-cat 300 10 false "WEST") false)
...
;; EXAMPLES : (on-key-event-helper-for-left (make-cat 100 100 true "SOUTH") (make-cat 300 10 false "WEST"))
;;            (make-world (make-cat 100 100 true "EAST") (make-cat 300 10 false "WEST") false)
;; STRATEGY : FUNCTIONAL COMPOSITION
...
;; EXAMPLES : (on-key-event-helper-for-left (make-cat 100 100 true "SOUTH") (make-cat 300 10 false "WEST"))
;;            (make-world (make-cat 100 100 true "NORTH") (make-cat 300 10 false "WEST") false)
;; STRATEGY : FUNCTIONAL COMPOSITION
...
(define(cat-change c)
  (make-cat (cat-x-pos c) (cat-y-pos c) true "WEST"))
...
(define (cat-after-button-up c x y)
  (if (cat-selected? c)
      (make-cat (cat-x-pos c) (cat-y-pos c) false (cat-dir c))
      c))
...
(define (initial-world y)
  (make-world
    (make-cat CAT1-X-COORD y false "EAST")
    (make-cat CAT2-X-COORD y false "EAST")
    false))
